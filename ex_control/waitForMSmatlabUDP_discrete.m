function [fixSuccess, bciSuccess, sentcursors,timevec ]= waitForMSmatlabUDP_discrete(waitTime,fixX,fixY,r,targX,targY,bcitarget, bcitolerance,bcifun,updatesintargetwin,cursorvisibleflag,cursorcolor,numcolors,cursorsize,bcirewardflag,ininds,delaytargpos,feedbackType,e,varargin)
...
%                     display(freqvals)
            ...
            if e.convergeAnnulus == 0 &&~isempty(freqevents)
                if cursorvisibleflag == 1
                    ...
                    switch feedbackType
                        case 'target'
                            for n = 1:length(ininds) % need to make sure targets in bci are in same order as here
                                if n == 1
                                    thismsg = sprintf('mset %i oval 0 %i %i %i %i %i %i',[ininds(n) delaytargpos(n,1) delaytargpos(n,2) cursorsize targetcolors(n,1) targetcolors(n,2) targetcolors(n,3)]);
                                else
                                    thismsg = sprintf(' set %i oval 0 %i %i %i %i %i %i',[ininds(n) delaytargpos(n,1) delaytargpos(n,2) cursorsize targetcolors(n,1) targetcolors(n,2) targetcolors(n,3)]);
                                end
                                msgstring = [msgstring,thismsg]  ;
                            end
                        case 'fixation'
                            msgstring = sprintf('mset %i oval 0 %i %i %i %i %i %i',[1 fixX fixY cursorsize targetcolors(bcitarget,1) targetcolors(bcitarget,2) targetcolors(bcitarget,3)]);
                        case 'annulus'
                            
                            targetPost = 1-posterior(bcitarget);
                            if isfield(e,'distfunflag')&&e.distfunflag == 1
                                
                                if min(posterior) ~= max(posterior)
                                    
%                                     % OLD ANNULUS SIZE (angle b/t bci target and posterior vector the hmm)
%                                     targetdist = abs(rad2deg(angle(sum(posterior.*complexAngles)/complexAngles(bcitarget)))/180);
                                    
                                    % NEW ANNULUS SIZE (projection of posterior vector onto unit vector pointing in direction of bci target)
                                    % 6/29/2018, Akash Umakantha
                                    postHmmVec = sum(posterior.*complexAngles);
                                    postHmmVec = [real(postHmmVec); imag(postHmmVec)];
                                    bciTargVec = complexAngles(bcitarget);
                                    bciTargVec = [real(bciTargVec); imag(bciTargVec)];
                                    projVal = postHmmVec'*bciTargVec; % between -1 (opposite dir, bad) and 1 (same direction, good)
                                    targetdist = (1-projVal)/2; % between 0 (good) and 1 (bad)
                                else
                                    newtargetdist = 0.5;
                                end

                                
                                %% rescale the mapping to annulus size to result in same annulus radius at threshold
                                % use piece-wise linear function
                                %mindist
                                %annulusSizeAtThresh 

                                if targetdist >= currmindist
                                    slope = (1-e.annulusSizeAtThresh)/(1-currmindist);
                                    b = 1- slope;
                                    newtargetdist = slope*targetdist+b;
                                else
                                    slope = e.annulusSizeAtThresh/currmindist;
                                    newtargetdist = slope*targetdist;
                                end
                                
                                
                                outerrad = round(e.minAnnulusSize+newtargetdist*(e.maxAnnulusSize-e.minAnnulusSize));
                            else
                                outerrad = round(e.minAnnulusSize+targetPost*(e.maxAnnulusSize-e.minAnnulusSize));
                            end
        ...
        if checkbcisuccessflag == 1
            ...
            checkbcisuccessflag = 0;
            if isfield(e,'distfunflag')&&e.distfunflag == 1
                bcifunout = bcifun(targetdist,currmindist);
            else
                bcifunout = bcifun(freqvals(end,:),bcitarget);
            end
