function [success, msgStr, fixWinOutput, extraFuncOutput] = bciMovementToTargetVmr(~, loopNow, bciSockets, binSizeMs, initCursorPos, initVelocity, pixBoxLimit, targX,targY,targRadius, cursorObjId, cursorR, cursorColor, targWinCursRad, vmrAngleDegrees)
...
if matlabUDP2('check', bciSockets.sender)
    ...
    if ~isempty(receivedMsg) && ~strcmp(receivedMsg, 'ack')
        ...
        % this approach is inherited from when different components of the
        % velocity-to-target vector were differently controlled; could've
        % just said velocityCurr=velocityNew, but decided to leave this
        % framework in in case we use it later
        centerToTargetVel = velocityNew' * normVecCenterToTarget;
        orthVel = velocityNew' * normVecOrth;
        ...
        % up messages that the loop can be tight--but the BCI computer
        % better be ready to receive what we're sending! (We do check for
        % an 'ack' to make sure things are running)
        try
            uint8Msg = typecast(velocityCurr, 'uint8');
        catch err
            a = err;
...
% here we introduce the VMR rotation--note that it DOESN'T change the
% velocity value, but JUST the position shift; so the vKF acts as if the
% velocity is going the nonrotated way, which is required for the rotated
% velocity to stay constant if the animal thinks nonrotated
rotationMat2d = [cosd(vmrAngleDegrees), -sind(vmrAngleDegrees);
                 sind(vmrAngleDegrees), cosd(vmrAngleDegrees)];
posPixelChange = rotationMat2d * velocityCurr * loopTimeDiff;
