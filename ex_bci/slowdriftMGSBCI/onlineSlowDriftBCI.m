...
% use original pupil calibration flag
useorigcalibflag = 0;
...
thismonth = num2str(month(date),'%02.0f');
thisday = num2str(day(date),'%02.0f');

filename = ['Pe',thisyear(3:4),thismonth,thisday,'_onlinedat'];



if useorigcalibflag == 1
    load([filepath,filename,'onlinedat1'])
    origmean = onlinedat(101).meanpupil;
    origstd = onlinedat(101).stdpupil;
    clear onlinedat
end
%% file parameter file 
...
% if isempty(filename)
%     filename = findBCIFileName(filepath,1);
% end
% thisyear = year(date);
...
if offlinemodeflag == 0 && pupilflag==0;[count]=xippmex('spike',elecs,[zeros(1,length(elecs))]);end
countbuffer = [];
trialstart = 0;
trialstartflag = 0;
num_bins = 1;
...
if pupilflag == 1
    fprintf(fileID,'\t%s','meanpupil');
    fprintf(fileID,'\t%s','stdpupil');
end
    ...
    while ~KbCheck
        ...
        if trialstart == 1
            if offlinemodeflag == 0
                thistic = tic;
                if pupilflag == 1
                    %display('I got to line 236 (don''t know why...')
                    [samples, timestamps] = xippmex('cont',elecs(4),10,'30ksps');%elecs 4 corresponds to pupil
                    countbuffertemp = mean(samples);
                    mintime = min(timestamps);
                    maxtime = mintime+length(samples)-1;
                    totalspikes = countbuffertemp;
                else
                ...
                if pupilflag==0 && savespiketimesflag == 1
            ...
            if sortflag ==1 && crossnoiseflag==0 && deepasortflag==0 && pupilflag==0
                goodcounts = cellfun(@(x)(sum(x~=0)),units);
            elseif sortflag ==0 && crossnoiseflag==1 && deepasortflag==0 && pupilflag==0
                [goodcounts, percentexclude] = excludeCrossChannelNoise(spiketimes,timebin,numevents);
            elseif sortflag ==0 && crossnoiseflag==0 && deepasortflag==1 && pupilflag==0
            ...
            elseif sortflag ==0 && crossnoiseflag==0 && deepasortflag==0 && pupilflag==1
            ...
            if pupilflag==0
                ...
                countbuffer = [countbuffer goodcounts];
            end
            ...
            if pupilflag == 1
                fprintf(fileID,'\t');
                fprintf(fileID,'%.10f',meanpupil);
                fprintf(fileID,'\t');
                fprintf(fileID,'%.10f',stdpupil);
            end
        ...
        while ((offlinemodeflag==1)&&(currmessagecount<=messagetotal)) || ((offlinemodeflag==0)&&matlabUDP2('check',sockind))
            ...
            elseif length(receivecommand)>=11&&strcmp('recalibrate',receivecommand(1:11))
               ...
               if pupilflag == 0
                   ...
                   [modelparams] = calibrateDistanceBCIFA(filepath,filepath,'filename',[filename,'_'],'dat',onlinedat,'modelparamsold',modelparamsold,'trialnum',numtrials,'dimsbci1',modelparams(1).dimsbci,'dimsbci2',modelparams(2).dimsbci); % need to fix the last two entries, these are bad assumptions
               else
                    allpupil = nan(length(onlinedat),1);
                    for n = 1:length(onlinedat)
                        if length(onlinedat(n).counts)>20
                            allpupil(n) = mean(onlinedat(n).counts((end-20+1):end));
                        end
                    end
                    if useorigcalibflag == 1
                        meanpupil = origmean;%nanmean(allpupil);
                        stdpupil = origstd;%nanstd(allpupil);
                    else
                        meanpupil = nanmean(allpupil);
                        stdpupil = nanstd(allpupil);                        
                    end
               end
               fprintf('Calibration finished!\n')
            elseif strcmp('pretrialstart',receivecommand)
                if offlinemodeflag == 0&&pupilflag==0; [~,~,~,units]= xippmex('spike',elecs,[zeros(1,length(elecs))]);end
            elseif length(receivecommand)>=8&&strcmp('trialend',receivecommand(1:8))
                ...
                if correctflag == 1
                    ...
                    display(mean(trackvals(end-20+1:end)))
                    ...
                    onlinedat(numtrials).meanpupil = meanpupil;
                    onlinedat(numtrials).stdpupil = stdpupil;
...
%                 if length(onlinedat)>60&&mod(length(onlinedat),10)==0
...
%                 end
        ...
        if trialstart == 1
            ...
            if pupilflag == 0
                ...
                valtosend = (countbuffer(end)-meanpupil)/stdpupil; % send mean pupil value, send normalized pupil, will be smoothed at control
            end
