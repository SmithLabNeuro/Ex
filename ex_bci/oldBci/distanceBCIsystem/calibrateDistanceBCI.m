function [modelparams, outdat] = calibrateDistanceBCI(filename,filepath,savepath,gamma,targetCorrect,multflag,offsetval,offlinemodeflag,trialnum,dat,modelparamsold,calibbin,numbins,waitbins)
% need filepath
% gamma
% offlinemodeflag
...
distancemetric = @(x,mu)((sum((x-repmat(mu,1,size(x,2))).^2,1)).^0.5);
...
allsmoothdist = [];
for n = 1:length(trainingdat)
    thiscount = trainingdat(n).counts;
    distances = distancemetric(thiscount,modelparams.mu);
...
%         smoothdist(scoreind) = meanvar.mu;
    ...
    trainingdat(n).distances = distances;
