...
clear all;
close all;
global debug sockets params thisTrialCodes
...
bciperiod = 0.05; %time in seconds
offbciperiod = 0.001;
looptimemaxerror = 0.01;
keepneurons = ones(96,1);
filename = 'Wa170122_s27a_dirmemwithhelp_0001';
timebin = 0.001;
numevents = 20;
%% Paths
addpath ../ex_control
...
load([filename,'modelparamstestgauss']) % rename this file
if offlinemodeflag == 1; load([filename,'spikingdata']); end
...
if offlinemodeflag == 0
    ...
    sockets = matlabUDP2('open','192.168.2.10','192.168.2.12',4245);
    ...
    while ~KbCheck
        ...
        while ((offlinemodeflag==1)&&(currmessagecount<=messagetotal)) || ((offlinemodeflag==0)&&matlabUDP2('check',sockind))        
            ...
            if strcmp('trialstart',receivecommand)
                ...
                correctflag = 0;
                testcursor = 0;
                sendtime = tic; % add buffer? otherwise there may be problems
                msg('obj_on 4')
            elseif strcmp('zerocursor',receivecommand)
                testcursor = 0;
            elseif strcmp('trialend',receivecommand)
                ...
                percentcorrect = nanmean(correcttrials);
...
%             this code is for a continuous decoder (KF)
            ...
            if offlinemodeflag==0
%            This code is for a discrete decoder followed by smoother             
%            matlabUDP2('send',sockind,num2str(meanvar1.mu)); % is this enough precision (~ 4 digits)?
%            matlabUDP2('send',sockind,num2str(meanvar2.mu));
%            fprintf('Cursor pos x: %d, y: %d sent!\n',meanvar1.mu,meanvar2.mu)
            
            
            %
            outpercent = (testcursor/10);
            while toc(sendtime)<bciperiod
            end
            sendtime = tic;
            if demoflag == 1
                sendtarg = round(outpercent*[targlocx targlocy]);
                %sendCode(codes.STIM1_ON);
                msg('dset 4 oval 0 %i %i %i %i %i %i',[sendtarg(1) sendtarg(2) 50 0 255 0]);
                %sendCode(codes.STIM2_ON);
                matlabUDP2('send',sockind,num2str(sendtarg));
            else
                msg('dset 4 oval 0 %i %i %i %i %i %i',[meanvar1.mu(1) meanvar1.mu(2) 50 0 255 0]);
                %sendCode(codes.stim1_on);
                matlabUDP2('send',sockind,num2str(meanvar1.mu));
            end
            
            testcursor = testcursor + 1;
            %fprintf('Cursor pos x: %d, y: %d sent!\n',meanvar.mu(1),meanvar.mu(2))
            end
...
%             subplot(1,2,2)
            ...
            if norm([min(abs(targlocx-curlocx)) min(abs(targlocy-curlocy))],2)<40
                correctflag = 1;
            end
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        % otherwise wait until loop time is finished
           while toc(countstime) < currlooptime
           end
%            if trialstart == 1
