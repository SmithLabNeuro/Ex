function calibrationBci(controlCompSocket, expParams, okelecs)
...
%                 decoderParameterFileRelativePath = 'satchel/21-Mar-2022/KalmanBci_14-02-09.mat';
                ...
                modelParams = load(decoderParameterFileFullPath, 'M0', 'M1', 'M2', 'channelsKeep');
                M0 = modelParams.M0;
                M1 = modelParams.M1;
                M2 = modelParams.M2;
...
%                 fprintf('received constrained velocity\n');
        ...
        if ~isempty(modelParams)
            ...
            if ~isempty(tstpBciStart)
                disp('bci start')
                timePtBciStarted = tmstp(tstpBciStart);
            ...
            if bciStart
                ...
                % calibration
                tmstp = tmstpInit(goodChannelInds);
                waveforms = waveforms(goodChannelInds);
                
                % run waveforms through NAS net
                tmstp = cellfun(@(wvForms, tms) tms(runNASNetContinuous(w1, b1, w2, b2, wvForms, gamma)), waveforms, tmstp, 'uni', 0);
%                 tmstp = cellfun(@(wvForms, tms) tms, waveforms, tmstp, 'uni', 0);
                spikesThisBinByChannel = cellfun(@(x) x>timePtBinStart & x<timePtBinStart+samplesPerBin, tmstp, 'uni', 0);
                %                 waveformsThisBinByChannel = cellfun(@(wvFrm, spksInBin) wvFrm(spksInBin, :), waveforms, spikesThisBinByChannel, 'uni', 0);
                spikesNextBinByChannel = cellfun(@(x) x>timePtBinStart+samplesPerBin, tmstp, 'uni', 0);
...
%                     fprintf('loop current is taking %0.3f secs\n', toc(loopTm));
                ...
                if any(allTmstps>(timePtBinStart+samplesPerBin))
                    ...
                    velocity = M0 + M1*velocity + M2 * meanSpikeCount;
