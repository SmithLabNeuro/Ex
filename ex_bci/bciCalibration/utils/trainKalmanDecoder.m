function [M0, M1, M2, channelsKeep, A, Q, C, R, beta, K] = trainKalmanDecoder(nev, trainParams, params, codes, datBase, nevBase,...
        netLabels, gamma, channelNumbersUse, Qvalue, numLatents, velocityToCalibrateWith, includeBaseForTrain)
    % Apply thresholding to data based on gamma
    slabel = zeros(size(netLabels)); %spike classification
    slabel(netLabels>=gamma) = 1;
    ...
    %Initialize Filtering parameters based on Firing Rate, the Fano Factor, and
    ...
    % Neural data that has been filtered for fano factor/firing
    % rate/coincidence and channels explicitly set by channelNumbersUse
    ...
    % Identify code for when cursor position moves 
    ...
    % Identify where BCI starts and ends (as soon as cursor movement starts) within calibration trials 
    ...
    spikeTimes = cellfun(@(firstSpike, spikeTimeDiffs) [firstSpike; uint64(firstSpike)+cumsum(uint64(spikeTimeDiffs))], {trimmedDat.firstspike}, {trimmedDat.spiketimesdiff}, 'uni', 0);
    ...
    % Identify trials that have cursor movements and are considered
    % training/valid calibration trials
    ...
    interpJoystickVelIntendedOnlyPos(bciValidTrials) = cellfun(...
        @(interpVel, interpPos, targLoc)...
        max(diag(interpVel * ((interpPos(end, :)-interpPos)./sqrt(sum((interpPos(end, :)-interpPos).^2,2)))'), 0, 'includenan').*((interpPos(end, :)-interpPos)./sqrt(sum((interpPos(end, :)-interpPos).^2,2))),... start with velocity 0, then find velocity in pix/s
        interpJoystickVel(bciValidTrials), interpJoystickPos(bciValidTrials), 'uni', 0);
    %%
    switch velocityToCalibrateWith
        case 'actual'
