function decoderFileLocationAndName = calibrateMainAndBackgroundKalmanVelocityDecoderForBci(~, nevFilebase, nevFilesForTrain, trainParams, subject)
% Function operates under assumption that two velocity decoders will be
% trained. 
global params codes
...
channelNumbersUseSecondary = trainParams.rippleChannelNumbersInSecondaryBci;
...
[trimmedDatSecondary, channelsKeepSecondary] = preprocessDat(datStruct, nevLabelledData, channelNumbersUseSecondary, binSizeMs, frThresh, ffThresh, coincThresh, coincTimeMs);
fprintf("\n%d channels being kept for secondary decoder\n", length(channelsKeepSecondary));
...
interpJoystickVelIntended(bciValidTrials) = cellfun(...
    @(interpVel, interpPos)...
    diag(interpVel * ((interpPos(end, :)-interpPos)./sqrt(sum((interpPos(end, :)-interpPos).^2,2)))').*((interpPos(end, :)-interpPos)./sqrt(sum((interpPos(end, :)-interpPos).^2,2))),... start with velocity 0, then find velocity in pix/s
