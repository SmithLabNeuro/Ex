...
if nargin >= 4
    ...
    while (toc(thisStart)*1000) <= waitTime
        ...
        if GetSecs - bciupdatetime >= bciloopintervaltemp
            ...
            if e.convergeAnnulus == 0 &&~isempty(freqevents)
                if cursorvisibleflag == 1
                    ...
                    targetdist = unitposition*e.annulusSize;
                    msgtime = toc(msgclock);
                    checkbcisuccessflag = 1;
                    display('test')
                    msgstring = sprintf('mset %i oval 0 %i %i %i %i %i %i',[4 round(centerX) round(centerY) e.cursorRad e.cursorColor(1) e.cursorColor(2) e.cursorColor(3)]);
                    if cursorvisibleflag == 1
        ...
        if checkbcisuccessflag == 1
            ...
            bcifunout = (((bcitargX-targetdist(1)).^2 + (bcitargY-targetdist(2)).^2)^0.5)>e.BCIDistanceThresh;%bcifun(targetdist,e.annulusSizeAtThresh);% the posterior should always be normalized so that correct = 1
