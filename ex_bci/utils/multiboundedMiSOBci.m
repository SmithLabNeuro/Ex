function multiboundedMiSOBci(controlCompSocket, expParams, okelecs)

global params codes m
...
% Define a physical shared memory space
filename = "/home/smithlab/Desktop/tmp/matlab_python_shm.dat";
input_dimensionality = expParams.ttlBinCntPre * length(expParams.targetLatentDim)+length(expParams.targetLatentDim);
% output_dimensionality = lengh(stimCandidateElecList); % output: suitability
% output_dimensionality = expParams.numStimElec; % output: stim pattern index
output_dimensionality = 1;
% Shared memory structure:
% flag(int32) | seq(int32) | algotype(int32) | target(double[3]) | input(double[18]) | output(int32[1]) | epsilon4(double) | epsilon5(double) | epsilon6(double)
m = memmapfile(filename, ...
    'Format', { ...
        'int32', [1 1], 'flag'; ...
        'int32', [1 1], 'seq'; ...
        'int32', [1 1], 'algotype'; ...
        'double', [1 length(expParams.targetLatentDim)], 'target'; ...
        'double', [1 input_dimensionality], 'input'; ... % num_pre_step*pre_dimensionality + targ_dimensionality
        'int32', [1 output_dimensionality], 'output'; ...
        'double', [1 1], 'epsilon4'; ... % epsilon for algo 4 (MiSO+)
        'double', [1 1], 'epsilon5'; ... % epsilon for algo 5 (OMiSO)
        'double', [1 1], 'epsilon6' ... % epsilon for algo 6 (OMiSO+)
    }, ...
    'Writable', true);
% Initialize
m.Data(1).flag = int32(0);
m.Data(1).seq  = int32(0);
m.Data(1).algotype  = int32(1);
m.Data(1).target  = double(zeros(1,length(expParams.targetLatentDim)));
m.Data(1).epsilon4 = double(expParams.epsilon); % initial epsilon for algo 4
m.Data(1).epsilon5 = double(expParams.epsilon); % initial epsilon for algo 5
m.Data(1).epsilon6 = double(expParams.epsilon); % initial epsilon for algo 6
...
digitalCodeNameInitStart = expParams.initStartCode;
digitalCodeInitStart = codes.(digitalCodeNameInitStart);
digitalCodeNameInitEnd = expParams.initEndCode;
digitalCodeInitEnd = codes.(digitalCodeNameInitEnd);
...
bciDecoderFunctionNameInit = strcat(bciDecoderFunctionName, 'Init');
% bciDecoderFunctionNameInit = strcat(bciDecoderFunctionName, 'InitWithModelMultiChanTorch');
...
% bciDecoderFunctionNamePre = strcat(bciDecoderFunctionName, 'PreWithModelMultiChanTorch');
bciDecoderFunctionNamePre = strcat(bciDecoderFunctionName, 'Pre');
% bciDecoderFunctionNamePost = strcat(expParams.name, 'Post');
% bciDecoderFunctionNamePost = strcat(expParams.name, 'PostWithModelMultiChanTorch');
bciDecoderFunctionNamePost = strcat(expParams.name, 'Post');
% bciDecoderFunction = str2func(bciDecoderFunctionName);
bciDecoderFunctionInit = str2func(bciDecoderFunctionNameInit);
...
while true
    ...
    tstpInitStart = find(prlEvents==digitalCodeInitStart);
    tstpInitEnd = find(prlEvents==digitalCodeInitEnd);
    ...
    if boundStarted      
        [modelParams, updatedReturn] = processMiSOControlMessage(controlCompSocket, ctrlMsg, modelParams);
...
%                 binSpikeCountNextOverall = zeros(length(goodChannelNums), 1);
            ...
            if ~isempty(tstpInitStart) && (isempty(tstpInitEnd))
                disp('initialization start')
                timePtInitStarted = tmstpPrlEvt(tstpInitStart);
                timePtInitStarted = timePtInitStarted(timePtInitStarted>=timePtBoundStarted);
                timePtInitBinStart = timePtInitStarted;
                initStart = true;
            end
            ...
            if ~isempty(tstpPostStart) && (isempty(tstpPostEnd))
                disp('post start')
            ...
            if ~isempty(tstpInitEnd)
                timePtInitEnd = tmstpPrlEvt(tstpInitEnd);
                timePtInitEnd = timePtInitEnd(timePtInitEnd>timePtBoundStarted);
            end
            ...
            if bciStart  
                ...
                if initStart
                    disp('init decoder')
                    currReturnInit = bciDecoderFunctionInit(currReturn, modelParams, expParams, controlCompSocket);

                    % prep the message to send
                    uint8Msg = typecast(currReturnInit, 'uint8');
                    ...
                    initStart = false;
                    if timePtInitEnd > timePtInitStarted
                        initStart = false;
                        currReturnInit = expParams.initReturn';
                        disp('init end')
                    end
                end
