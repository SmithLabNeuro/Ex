function [trimmedDat, channelsKeep, stimChanFlat] = getChannelsKeepWithDat(datStruct, nevLabelledData, channelNumbersUse, binSizeMs, frThresh, ffThresh, coincThresh, coincidenceTimeMs)
...
% SKIP: extract no uStim trials
% TODO: write using the regular expression
% stimChan = cellfun(@(dS) regexp(dS, 'xippmexStimChan*=((\d*\s*)*);.*', 'tokens'), {datStruct.text});
% stimChan = cellfun(@(dS) regexp(dS, 'xippmexStimChanUpdated*=((\d*\s*)*);.*', 'tokens'), {datStruct.text}, 'uni', 0);
stimChan = cellfun(@(dS) ...
    getStimChanOrZero(dS), ...
    {datStruct.text}, ...
    'UniformOutput', false);

% Convert cell array of strings to string array (preserves full electrode strings like "33 45 50 62")
stimChanFlat = string(stimChan);
% stimChan = cellfun(@(a) a{1}, stimChan, 'UniformOutput', false);
datStruct = datStruct(strcmp(stimChanFlat, '0')|strcmp(stimChanFlat, '0  0')|strcmp(stimChanFlat, '0  0  0')|strcmp(stimChanFlat, '0  0  0  0')|strcmp(stimChanFlat, '0  0  0  0  0'));
...
function out = getStimChanOrZero(dS)
    tok = regexp(dS, 'xippmexStimChanUpdated*=((\d*\s*)*);', 'tokens', 'once');
    if isempty(tok)
        out = '0';  % Return string '0' instead of numeric 0
    else
        out = strtrim(tok{1});  % Keep as string, don't convert to double
    end
end
