function stim_fef_dotsPost(optstr,w,objID,arg)
...
if strcmp(optstr,'setup')
    ...
    if numel(posPost)<size(dotPositions,3)
        rng(seed);
        posPost=[posPost, randperm(xGrid*yGrid)];
    end
    
    for i = 1:size(dotPositions,3)
        
        xPos = mod(posPost(1:numDots)-1,16)+1;
        yPos = floor((posPost(1:numDots)-1)/16)+1;
