function stim_fef_dotsPre(optstr,w,objID,arg)
...
if strcmp(optstr,'setup')
    a = sscanf(arg,'%i %i %i %f %i %i %i %i %i %i %i %i %i');
    
    % arguments: (1) frameCount
    %            (2) random seed
    %            (3) number of dots per frameposPre(1:numDots)
    %            (4) dot size
    %            (5) dwell
    %            (6) center x
    %            (7) center y
    ...
    listLength = ceil(a(13)/(xGrid*yGrid))*xGrid*yGrid;
    
    positionsList=zeros(listLength,1);
    rng(seed);
    for i=1:ceil(a(13)/(xGrid*yGrid))
        positionsList((i-1)*xGrid*yGrid+1:i*xGrid*yGrid)=randperm(xGrid*yGrid);
    end
    
    for i = 1:size(dotPositions,3)
        
        indexes=(trialNum-1)*size(dotPositions,3)+i:(trialNum-1)*size(dotPositions,3)+i+numDots-1;
        indexes=mod(indexes-1,listLength)+1;
        
        xPos = mod(positionsList(indexes)-1,xGrid)+1;
        yPos = floor((positionsList(indexes)-1)/xGrid)+1;
